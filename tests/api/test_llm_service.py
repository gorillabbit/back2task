...
class TestLLMService:
    ...
    def llm_service(self):
        ...
        return LLMService(base_url="http://localhost:8000", model_name="gpt-oss-20b")
    ...
    def test_llm_service_initialization(self, llm_service):
        """LLMサービスの初期化テスト"""
        assert llm_service.base_url == "http://localhost:8000"
        assert llm_service.model_name == "gpt-oss-20b"
        assert llm_service.timeout == 20.0
        assert llm_service.chat_url == "http://localhost:8000/v1/chat/completions"
    ...
    def test_fallback_policy_strong_distraction(self, llm_service, distracted_observations):
        """フォールバック政策（強い脱線）"""
    ...
    def test_fallback_policy_phone_distraction(self, llm_service, phone_distracted_observations):
        """フォールバック政策（スマホ脱線）"""
    ...
    def test_decide_nudging_policy_llm_unavailable(self, llm_service, mock_observations):
        """LLM利用不可時のnudging決定"""
        with patch.object(llm_service, 'is_available', return_value=False):
            policy = llm_service.decide_nudging_policy("テストタスク", mock_observations)
    ...
    def test_decide_nudging_policy_llm_json_parse_error(self, llm_service, mock_observations):
        """LLM JSONパースエラー時の処理"""
        with patch.object(llm_service, 'is_available', return_value=True), \
             patch('requests.post') as mock_post:
    ...
    def test_generate_task_suggestions_llm_success(self, llm_service):
        ...
        with patch.object(llm_service, 'is_available', return_value=True), \
            ...
            assert suggestions["estimated_minutes"] == mock_response_data["estimated_minutes"]
    ...
    def test_rate_limiting(self, llm_service):
        ...
        # 2回目の呼び出しで待機時間が発生する
        assert second_call_time >= llm_service.min_call_interval - 0.1  # 少し余裕を持たせる
    ...
    def test_get_model_info_failure(self, llm_service):
        ...
        with patch('requests.get') as mock_get:
            ...
            assert info["data"][0]["id"] == "gpt-oss-20b"
    ...
    def test_convenience_functions(self):
        ...
        # create_llm_service
        service = create_llm_service("http://test:8000")
