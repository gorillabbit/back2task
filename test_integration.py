...
from api.services.llm import LLMService, NudgingPolicy
...
class Back2TaskIntegrationTest:
    ...
    def test_api_availability(self) -> bool:
        ...
        try:
            ...
            print("ヒント: uvicorn api.main:app --reload --port 5577 で起動してください")
            return False
    ...
    def test_llm_availability(self) -> bool:
        ...
        try:
            ...
            print("ヒント: vLLMサーバーを起動してください")
            print("python -m vllm.entrypoints.openai.api_server --model openai/gpt-oss-20b --port 8000")
            return False
    ...
    def test_session_lifecycle(self) -> bool:
        ...
        try:
            ...
            for i, event in enumerate(test_events):
                ...
                print(f"✓ イベント{i+1}送信・判定成功 (生産的: {result['productive']})")
    ...
    def test_llm_integration(self) -> bool:
        ...
        try:
            ...
            for case in test_cases:
                policy = llm_service.decide_nudging_policy("テストタスク", case["observations"])
    ...
    def test_watchers_data_collection(self) -> bool:
        ...
        try:
            ...
            required_fields = ["active_app", "title", "idle_ms", "ocr", "phone_detected"]
    ...
    def test_end_to_end_scenario(self) -> bool:
        ...
        try:
            ...
            for i, scenario in enumerate(scenarios):
                ...
                # イベント送信
                response = requests.post(f"{self.api_url}/events", json=scenario["event"])
                assert response.status_code == 200
                
                result = response.json()
                actual_productive = result.get("productive")
                
                print(f"    生産性判定: {actual_productive} (期待: {scenario['expected_productive']})")
                
                if actual_productive != scenario["expected_productive"]:
                    print(f"    ⚠️  判定が期待と異なります")
                
                # LLMでnudging判定
                policy = llm_service.decide_nudging_policy("e2e_test", scenario["event"])
                ...
                if not actual_productive:
                    if policy.action == "gentle_nudge":
                        notification_service.notify(
                            "軽い注意", policy.tip or "作業に戻りましょう",
                            NotificationLevel.WARNING
                        )
                    elif policy.action == "strong_nudge":
                        notification_service.notify(
                            "強い注意", policy.tip or "今は作業時間です！",
            ...
            print(f"  積算時間: {final_status.get('current_task', {}).get('accum', 0)}秒")
            print(f"  現在の状態: {'生産的' if final_status.get('productive') else '非生産的'}")
    ...
    def print_summary(self, results: Dict[str, bool]):
        ...
        if passed == total:
            ...
            if not results.get("API可用性"):
                print("- FastAPIサーバーを起動: uvicorn api.main:app --reload --port 5577")
            
            if not results.get("LLM可用性"):
                print("- vLLMサーバーを起動: python -m vllm.entrypoints.openai.api_server --model openai/gpt-oss-20b --port 8000")
            
            if not results.get("Watchersデータ収集"):
                print("- 必要な依存関係をインストール: pip install opencv-python pytesseract ultralytics")
