...
""".strip()
    ...
    def _build_context_prompt(self, task: str, observations: Dict[str, Any]) -> str:
        ...
        phone_detected = observations.get("phone_detected", False)
        phone = observations.get("phone") or ""
        ...
        # スマホ検出の説明
        phone_desc = "phone detected" if phone_detected else "no phone"
        if phone and phone != "":
            phone_desc += f" ({phone})"
        
        # OCRテキストを要約（長すぎる場合）
        ocr_summary = ocr[:100] + "..." if len(ocr) > 100 else ocr
...
- Status: {idle_desc}
- Phone: {phone_desc}
- Screen Text: {ocr_summary if ocr_summary else 'N/A'}
...
""".strip()
    ...
    def decide_nudging_policy(self, task: str, observations: Dict[str, Any]) -> NudgingPolicy:
        """
    ...
    def _fallback_policy(self, observations: Dict[str, Any]) -> NudgingPolicy:
        ...
        """
        active_app = observations.get("active_app", "").lower()
        title = observations.get("title", "").lower()
        url = observations.get("url", "").lower()
        idle_ms = observations.get("idle_ms", 0)
        phone_detected = observations.get("phone_detected", False)
        ocr = observations.get("ocr", "").lower()
        
        # 強い脱線パターン
        strong_distractions = [
            "youtube", "tiktok", "twitter", "instagram", "facebook",
            "netflix", "prime video", "steam", "game"
        ]
        ...
        # 生産的パターン
        productive_patterns = [
            "code", "vscode", "vim", "emacs", "ide", "terminal",
            "documentation", "github", "stackoverflow", "programming"
        ]
        
        # 検出ロジック
        content_to_check = f"{active_app} {title} {url} {ocr}"
        ...
        if phone_detected:
            return NudgingPolicy(
                ...
                tip="Focus on your current task",
                confidence=0.9
            )
...
def decide_nudging_policy(task: str, observations: Dict[str, Any], 
                         llm_service: Optional[LLMService] = None) -> NudgingPolicy:
